...
const EMP_COLS = {
  TIMESTAMP: 0, EMP_ID: 1, NAME: 2, LOCATION: 3,
  PAN: 4, BANK_NAME: 5, BENEFICIARY: 6,
  ACCOUNT_NO: 7, IFSC: 8, REG_NO: 9,
};
...
const REQ_COLS = {
  ...
  EXPECTED_INVOICE_DATE: 17, STATUS: 18, CREATED_AT: 19,
};
...
function getUserInfo() {
  ...
  try {
    ...
    console.log("Error getting active user:", e);
  }
...
function getAllRequests() {
  ...
  // Sort newest first by raw CREATED_AT timestamp
  requests.sort((a, b) => {
    const ta = a._createdRaw ? new Date(a._createdRaw).getTime() : 0;
    const tb = b._createdRaw ? new Date(b._createdRaw).getTime() : 0;
    return tb - ta;
  });
  return requests;
}
...
function rowToRequest(row) {
  return {
    id                  : row[REQ_COLS.ID],
    timestamp           : row[REQ_COLS.TIMESTAMP]
                            ? new Date(row[REQ_COLS.TIMESTAMP]).toLocaleString("en-IN") : "",
    requestor           : row[REQ_COLS.REQUESTOR],
    contact             : row[REQ_COLS.CONTACT],
    email               : row[REQ_COLS.EMAIL],
    dept                : row[REQ_COLS.DEPT],
    expenseType         : row[REQ_COLS.EXPENSE_TYPE],
    category            : row[REQ_COLS.CATEGORY],
    description         : row[REQ_COLS.DESCRIPTION],
    amount              : row[REQ_COLS.AMOUNT],
    date                : row[REQ_COLS.DATE]
                            ? new Date(row[REQ_COLS.DATE]).toLocaleDateString("en-IN") : "",
    payeeName           : (row[REQ_COLS.PAYEE_NAME] || "").toString().trim(),
    payeeType           : (row[REQ_COLS.PAYEE_TYPE] || "").toString().trim(),
    upi                 : row[REQ_COLS.UPI],
    refId               : row[REQ_COLS.REF_ID],
    invoiceUrl          : row[REQ_COLS.INVOICE_URL],
    advanceReason       : row[REQ_COLS.ADVANCE_REASON],
    expectedInvoiceDate : row[REQ_COLS.EXPECTED_INVOICE_DATE]
                            ? new Date(row[REQ_COLS.EXPECTED_INVOICE_DATE]).toLocaleDateString("en-IN") : "",
    status              : row[REQ_COLS.STATUS],
    createdAt           : row[REQ_COLS.CREATED_AT]
                            ? new Date(row[REQ_COLS.CREATED_AT]).toLocaleString("en-IN") : "",
    _createdRaw         : row[REQ_COLS.CREATED_AT] || row[REQ_COLS.TIMESTAMP] || "",
  };
}
...
function getApprovalLogs(reqId) {
  ...
  const logs = [];
  for (let i = 1; i < data.length; i++) {
    if (data[i][APPROVAL_COLS.REQUEST_ID] === reqId) {
      logs.push({
        logId    : data[i][APPROVAL_COLS.LOG_ID],
        requestId: data[i][APPROVAL_COLS.REQUEST_ID],
        action   : data[i][APPROVAL_COLS.ACTION],
        byName   : data[i][APPROVAL_COLS.BY_NAME],
        byEmail  : data[i][APPROVAL_COLS.BY_EMAIL],
        remarks  : data[i][APPROVAL_COLS.REMARKS],
        timestamp: new Date(data[i][APPROVAL_COLS.TIMESTAMP]).toLocaleString("en-IN"),
      });
    }
  }
  return logs;
...
function getPaymentLogs(reqId) {
  ...
  const logs = [];
  for (let i = 1; i < data.length; i++) {
    if (data[i][PAYMENT_COLS.REQUEST_ID] === reqId) {
      logs.push({
        logId        : data[i][PAYMENT_COLS.LOG_ID],
        requestId    : data[i][PAYMENT_COLS.REQUEST_ID],
        utr          : data[i][PAYMENT_COLS.UTR],
        paymentAmount: data[i][PAYMENT_COLS.PAYMENT_AMOUNT],
        paidByName   : data[i][PAYMENT_COLS.PAID_BY_NAME],
        paidByEmail  : data[i][PAYMENT_COLS.PAID_BY_EMAIL],
        paymentDate  : data[i][PAYMENT_COLS.PAYMENT_DATE]
                         ? new Date(data[i][PAYMENT_COLS.PAYMENT_DATE]).toLocaleDateString("en-IN") : "",
        mode         : data[i][PAYMENT_COLS.MODE],
        remarks      : data[i][PAYMENT_COLS.REMARKS],
        timestamp    : new Date(data[i][PAYMENT_COLS.TIMESTAMP]).toLocaleString("en-IN"),
      });
    }
  }
  return logs;
