/**
 * Username/email + password login (session token). No Google-account matching.
 */

var SESSION_PREFIX = "ke_inv_sess_";
...
function passwordMatchesStored_(stored, plain, inputHash) {
  ...
  if (stored === inputHash || stored === plain) {
    return true;
  }
  return false;
}
...
function loginUser(identifier, password) {
  ...
  if (cEmail < 0) {
    throw new Error("USER_MASTER must have an Email column.");
  }
  if (cPass < 0) {
    throw new Error("USER_MASTER must have a Password column.");
  }
  ...
  for (var i = 0; i < data.rows.length; i++) {
    ...
    var idLower = identifier;
    var matches =
      email.toLowerCase() === idLower ||
      (name && name.toLowerCase() === idLower) ||
      (name && name.toLowerCase().replace(/\s+/g, "") === idLower.replace(/\s+/g, "")) ||
      emailsMatch_(email, idLower);
    if (!matches) {
      continue;
    }
    if (!findMatchingPasswordInRow_(row, password, inputHash, cPass)) {
...
/**
 * Run once: hashUserPassword("KE Admin", "admin@123");
 */
