...
try{
    const ns = sendMailKE_(primaryAdmin, 'KE Nightly Summary · ' + (CONFIG.LOCATION_CITY || 'Hyderabad') + ': ' + todayLbl, htmlBody, {
      attachments : [pdfBlob],
      cc          : ccAdmins.join(',')
    });
      logEmail('NightlySummary', primaryAdmin, ccAdmins.join(','), 'KE Nightly Summary · ' + (CONFIG.LOCATION_CITY || 'Hyderabad') + ': ' + todayLbl, '', 'Sent', 'via ' + ns.provider);
} catch (e) {
  logEmailFailed('NightlySummary', primaryAdmin, ccAdmins.join(','), 'KE Nightly Summary · ' + (CONFIG.LOCATION_CITY || 'Hyderabad') + ': ' + todayLbl, '', String(e));
}
...
<div style="max-width:860px;margin:20px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 14px rgba(0,0,0,.1)">
  <div style="background:linear-gradient(135deg,#1f4e3d,#4f9c7a);padding:26px 30px;display:flex;align-items:center;gap:16px">
   ...
   style="width:72px;height:72px;border-radius:50%;border:3px solid #000;margin-bottom:12px"> 
    <div>
      ...
      <p style="margin:4px 0 0;color:rgba(255,255,255,.88);font-size:13px">Kings Equestrian Foundation — ${schoolLocationShort_()}</p>
  ...
  <div style="background:#1f4e3d;color:#fff;padding:16px 30px;text-align:center;font-size:12px">
    <strong>Kings Equestrian Foundation</strong> | ${schoolLocationFull_()} | ${CONFIG.CONTACT_PHONE || '+91-9980895533'} | ${CONFIG.CONTACT_EMAIL || CONFIG.MAIL_FROM || ''}
  </div>
</div></body></html>`;
}
...
<div class="meta">${schoolLocationFull_()} · Generated: ${reportDate}</div>
<div class="stats">
...
<div class="footer">${emailFooterPlain_()}</div>
</body></html>`;
