...
/**
 * Main Web App entrypoint for version4.
 *
 * - Default (no params): Staff Attendance portal (V2) — includes regular riders
 * - `?app=portal`: Rider Portal
 *
 * This removes the need for `patches.js` routing.
 */
function doGet(e) {
    var app = (e && e.parameter && e.parameter.app) ? String(e.parameter.app) : 'attendance';
    if (app === 'portal') {
        // Rider portal (regularPortal.js)
        return doGetPortal(e);
    }

    // Staff attendance (attendanceV2.js)
    // Use template evaluation (more reliable than createHtmlOutput in some deployments)
    const template = HtmlService.createTemplate(getAttendanceAppV2Html());
    return template
        .evaluate()
