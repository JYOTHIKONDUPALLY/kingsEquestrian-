...
const CONFIG = {
    ...
    EMAIL_TEMPLATE_PDF_ID: "d/17t23GLXC8g8MkdCDx4BmMswuzrdYM_x0",
    ...
    RIDER_PORTAL_URL: "https://script.google.com/macros/s/AKfycbzqVMspYfVAEWnlsJBjmo_vEKqoXotS3CMpQMRmAnOHSjnrZzY5F9p3EbSRKm2lKZ1C/exec",
    ...
    SHEETS: {
        ...
        PRICING: "Pricing",
        MAIL_INFO: "Mail Info",
        REGULAR_BOOKING_FORM: "RegularRidersBooking",
...
function getPdfTemplate() {
    try {
        if (!CONFIG.EMAIL_TEMPLATE_PDF_ID) return null;  // ← add this guard
...
/**
 * Finds the most recent booking row matching a booking reference (KE...).
 * Returns { rowIndex, row } or null.
 */
function findLatestBookingByReference(reference, bookingValues) {
    const ref = String(reference || '').trim();
    if (!ref) return null;
    for (let j = bookingValues.length - 1; j >= 1; j--) {
        const rowRef = String(bookingValues[j][CONFIG.BOOKING_COLS.REFERENCE] || '').trim();
        if (rowRef === ref) return { rowIndex: j + 1, row: bookingValues[j] };
    }
    return null;
}

function _normalizePhone10(phone) {
    const digits = String(phone || '').replace(/\D/g, '');
    return digits.length > 10 ? digits.slice(-10) : digits;
}

function findRegularStudentByPhone(ss, phone) {
    const studentSheet = ss.getSheetByName(CONFIG.SHEETS.REGULAR_STUDENTS);
    if (!studentSheet) return null;
    const target = _normalizePhone10(phone);
    if (!target || target.length < 10) return null;

    const data = studentSheet.getDataRange().getValues();
    for (let i = 1; i < data.length; i++) {
        const rowPhone = _normalizePhone10(data[i][CONFIG.STUDENT_COLS.PHONE]);
        if (rowPhone === target) return { rowIndex: i + 1, row: data[i] };
    }
    return null;
}

function findRegularStudentByRegNo(ss, regNo) {
    const studentSheet = ss.getSheetByName(CONFIG.SHEETS.REGULAR_STUDENTS);
    if (!studentSheet) return null;
    ...
    const data = studentSheet.getDataRange().getValues();
    for (let i = 1; i < data.length; i++) {
        const rowReg = String(data[i][CONFIG.STUDENT_COLS.REG_NO] || '').trim();
        if (rowReg === target) return { rowIndex: i + 1, row: data[i] };
    }
    return null;
}

function applyRegularPaymentToStudent(ss, regNo, amount, transactionId) {
    const studentSheet = ss.getSheetByName(CONFIG.SHEETS.REGULAR_STUDENTS);
    if (!studentSheet) return { updated: false, error: 'Regular Students sheet not found' };

    const match = findRegularStudentByRegNo(ss, regNo);
    if (!match) return { updated: false, error: 'Regular student not found for reg no' };

    const rowIndex = match.rowIndex;
    const row = match.row;
    const addAmt = Number(amount) || 0;
    if (!addAmt || addAmt <= 0) return { updated: false, error: 'Invalid amount' };

    const currentPaid = Number(row[CONFIG.STUDENT_COLS.AMOUNT_PAID]) || 0;
    const totalAmt = Number(row[CONFIG.STUDENT_COLS.TOTAL_AMOUNT]) || 0;
    const newPaid = currentPaid + addAmt;

    let status = 'Pending';
    if (totalAmt > 0) {
        if (newPaid >= totalAmt) status = 'Paid';
        else if (newPaid > 0) status = 'Partial';
    } else {
        status = newPaid > 0 ? 'Paid' : 'Pending';
    }

    // Update Amount Paid + Payment Status
    studentSheet.getRange(rowIndex, CONFIG.STUDENT_COLS.AMOUNT_PAID + 1).setValue(newPaid);
    studentSheet.getRange(rowIndex, CONFIG.STUDENT_COLS.PAYMENT_STATUS + 1).setValue(status);

    // Append payment ref if present
    if (transactionId) {
        const existingRef = String(row[CONFIG.STUDENT_COLS.PAYMENT_REF] || '').trim();
        const tid = String(transactionId).trim();
        const nextRef = existingRef
            ? (existingRef.includes(tid) ? existingRef : existingRef + ', ' + tid)
            : tid;
        studentSheet.getRange(rowIndex, CONFIG.STUDENT_COLS.PAYMENT_REF + 1).setValue(nextRef);
    }

    return { updated: true, newPaid: newPaid, totalAmt: totalAmt, status: status };
}
...
function resendExistingReceipt(currentRow, existingRow) {
    try {
        ...
        const registrationNo = String(currentData[CONFIG.PAYMENT_COLS.REGISTRATION_NO] || '').trim();
        ...
        // Resolve rider either from Booking Form (one-time) or Regular Students
        const bookingValues = bookingSheet ? bookingSheet.getDataRange().getValues() : [];
        let bookingMatch = null;
        if (registrationNo && bookingValues.length) {
            bookingMatch = findLatestBookingByReference(registrationNo, bookingValues);
        }
        if (!bookingMatch && bookingValues.length) {
            bookingMatch = findLatestBookingByPhone(phoneNumber, bookingValues);
        }

        let riderName = '';
        let email = '';
        let referenceNumber = registrationNo;
        let preferredDate = '';
        ...
        if (bookingMatch) {
            ...
            referenceNumber = String(bookingMatch.row[CONFIG.BOOKING_COLS.REFERENCE] || registrationNo).trim();
            ...
            const regStudentMatch = registrationNo
                ? findRegularStudentByRegNo(ss, registrationNo)
                : findRegularStudentByPhone(ss, phoneNumber);
            if (!regStudentMatch) throw new Error(`No matching booking/regular rider found for phone ${phoneNumber}${registrationNo ? ' (Reg: ' + registrationNo + ')' : ''}`);
            riderName = regStudentMatch.row[CONFIG.STUDENT_COLS.NAME] || '';
            email = regStudentMatch.row[CONFIG.STUDENT_COLS.EMAIL] || '';
            referenceNumber = String(regStudentMatch.row[CONFIG.STUDENT_COLS.REG_NO] || registrationNo).trim();
        }

        if (!email) throw new Error('Email not found for this rider');
...
+91-9980895533 | info@kingsequestrian.com
  `;
   ...
   if (data.sheet && data.row) {
    const isRegular = data.sheet.getName() === CONFIG.SHEETS.REGULAR_BOOKING_FORM;
    const sentCol      = isRegular ? CONFIG.REG_BOOKING_COLS.REG_EMAIL_SENT      : CONFIG.BOOKING_COLS.WELCOME_EMAIL_SENT;
    const timestampCol = isRegular ? CONFIG.REG_BOOKING_COLS.REG_EMAIL_TIMESTAMP : CONFIG.BOOKING_COLS.WELCOME_EMAIL_TIMESTAMP;

    data.sheet.getRange(data.row, sentCol + 1)
        .setValue('Yes')
        .setBackground('#d4edda')
        .setFontColor('#155724')
        .setFontWeight('bold');

    data.sheet.getRange(data.row, timestampCol + 1)
        .setValue(new Date())
        .setNumberFormat('dd-MMM-yyyy HH:mm:ss');
}
...
function sendReceiptForRow(rowIndex) {
    ...
    try {
        ...
        const registrationNo = String(row[CONFIG.PAYMENT_COLS.REGISTRATION_NO] || '').trim();

        if (!phoneNumber) throw new Error('Phone number missing in payment form');

        // Resolve payer as either one-time booking OR regular rider
        let payerType = '';
        let riderName = '';
        ...
        let preferredDate = '';
        let preferredTimeSlots = '';
        let referenceNumber = registrationNo; // fallback

        // 1) If registration number is provided, try to match booking reference first
        let bookingMatch = null;
        if (registrationNo && bookingValues && bookingValues.length) {
            bookingMatch = findLatestBookingByReference(registrationNo, bookingValues);
        }
        ...
        if (bookingMatch) {
            ...
            riderName = bookingMatch.row[CONFIG.BOOKING_COLS.NAME] || '';
            email = bookingMatch.row[CONFIG.BOOKING_COLS.EMAIL_ID] || '';
            phone = bookingMatch.row[CONFIG.BOOKING_COLS.PHONE_NUMBER] || phoneNumber;
            ...
            // 2) Try regular rider (by registration no, then by phone)
            const regStudentMatch = registrationNo
                ? findRegularStudentByRegNo(ss, registrationNo)
                : findRegularStudentByPhone(ss, phoneNumber);
            ...
            if (regStudentMatch) {
                ...
                riderName = regStudentMatch.row[CONFIG.STUDENT_COLS.NAME] || '';
                email = regStudentMatch.row[CONFIG.STUDENT_COLS.EMAIL] || '';
                phone = regStudentMatch.row[CONFIG.STUDENT_COLS.PHONE] || phoneNumber;
                services = regStudentMatch.row[CONFIG.STUDENT_COLS.PROGRAM] || '';
                participants = regStudentMatch.row[CONFIG.STUDENT_COLS.PARTICIPANTS] || 1;
                referenceNumber = String(regStudentMatch.row[CONFIG.STUDENT_COLS.REG_NO] || registrationNo).trim();
            } else {
                // 3) Fallback: latest booking by phone (one-time)
                if (bookingValues && bookingValues.length) {
                    bookingMatch = findLatestBookingByPhone(phoneNumber, bookingValues);
                }
                if (!bookingMatch) {
                    throw new Error(`No matching booking/regular rider found for phone ${phoneNumber}${registrationNo ? ' (Reg: ' + registrationNo + ')' : ''}`);
                }
                payerType = 'one-time';
                riderName = bookingMatch.row[CONFIG.BOOKING_COLS.NAME] || '';
                email = bookingMatch.row[CONFIG.BOOKING_COLS.EMAIL_ID] || '';
        ...
        // If this payment is for a regular rider, update Regular Students ledger
        if (payerType === 'regular' && referenceNumber) {
            try {
                applyRegularPaymentToStudent(ss, referenceNumber, amount, transactionId);
            } catch (e) {
                Logger.log('Regular payment ledger update failed (non-fatal): ' + e);
            }
        }
...
function onRegularBookingFormSubmit(e) {
    try {
        const sheet = e.range.getSheet();
        if (sheet.getName() !== CONFIG.SHEETS.REGULAR_BOOKING_FORM) return;
        
        const row = e.range.getRow();
        const name  = sheet.getRange(row, CONFIG.REG_BOOKING_COLS.NAME + 1).getValue();
        const email = sheet.getRange(row, CONFIG.REG_BOOKING_COLS.EMAIL_ID + 1).getValue();
        const phone = sheet.getRange(row, CONFIG.REG_BOOKING_COLS.PHONE_NUMBER + 1).getValue();
        const services = sheet.getRange(row, CONFIG.REG_BOOKING_COLS.OUR_SERVICES + 1).getValue();
        const participants = Number(sheet.getRange(row, CONFIG.REG_BOOKING_COLS.NUMBER_OF_PARTICIPANTS + 1).getValue()) || 1;
        const bookingDate = sheet.getRange(row, CONFIG.REG_BOOKING_COLS.TIMESTAMP + 1).getValue();
        ...
        sheet.getRange(row, CONFIG.REG_BOOKING_COLS.REG_NO + 1).setValue(reference);
        ...
        // Mark email sent
        sheet.getRange(row, CONFIG.REG_BOOKING_COLS.REG_EMAIL_SENT + 1)
            .setValue('Yes').setBackground('#d4edda').setFontColor('#155724').setFontWeight('bold');
        sheet.getRange(row, CONFIG.REG_BOOKING_COLS.REG_EMAIL_TIMESTAMP + 1)
            .setValue(new Date()).setNumberFormat('dd-MMM-yyyy HH:mm:ss');
createRegularStudentRecord(sheet, row, reference);

    } catch (error) {
        Logger.log('Error in onRegularBookingFormSubmit: ' + error);
    }
}
function createRegularStudentRecord(sheet, row, reference) {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const studentSheet = ss.getSheetByName(CONFIG.SHEETS.REGULAR_STUDENTS);
    if (!studentSheet) {
        Logger.log('Regular Students sheet not found');
        return;
    }
    ...
    const phone        = sheet.getRange(row, CONFIG.REG_BOOKING_COLS.PHONE_NUMBER + 1).getValue();
    const services     = sheet.getRange(row, CONFIG.REG_BOOKING_COLS.OUR_SERVICES + 1).getValue();
    const participants = sheet.getRange(row, CONFIG.REG_BOOKING_COLS.NUMBER_OF_PARTICIPANTS + 1).getValue();

    // Check if student already exists by phone
    const existingData = studentSheet.getDataRange().getValues();
    for (let i = 1; i < existingData.length; i++) {
        const existingPhone = String(existingData[i][CONFIG.STUDENT_COLS.PHONE] || '').replace(/\D/g, '');
        const newPhone      = String(phone || '').replace(/\D/g, '');
        if (existingPhone === newPhone) {
            Logger.log('Student already exists: ' + phone);
            return;
        }
    }
    const pricingSheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(CONFIG.SHEETS.PRICING);
const pricingData  = pricingSheet ? pricingSheet.getDataRange().getValues() : [];

let totalClasses  = 0;
let totalAmount   = 0;

for (let p = 1; p < pricingData.length; p++) {
    const pricingService = String(pricingData[p][1] || '').trim().toLowerCase();
    const bookingService = String(services || '').trim().toLowerCase();
    if (bookingService.includes(pricingService) || pricingService.includes(bookingService)) {
        // Find columns by header name
        const headers = pricingData[0];
        const classesCol      = headers.findIndex(h => String(h).toLowerCase().includes('classes'));
        const packagePriceCol = headers.findIndex(h => String(h).toLowerCase().includes('packageprice') || String(h).toLowerCase().includes('package price'));
        if (classesCol !== -1)      totalClasses = Number(pricingData[p][classesCol])      || 0;
        if (packagePriceCol !== -1) totalAmount  = Number(pricingData[p][packagePriceCol]) || 0;
        break;
    }
}
    ...
    newRow[CONFIG.STUDENT_COLS.REG_NO]       = reference;
    newRow[CONFIG.STUDENT_COLS.NAME]         = name;
    newRow[CONFIG.STUDENT_COLS.EMAIL]        = email;
    newRow[CONFIG.STUDENT_COLS.PHONE]        = phone;
    newRow[CONFIG.STUDENT_COLS.PROGRAM]      = services;
    newRow[CONFIG.STUDENT_COLS.PARTICIPANTS] = participants;
    newRow[CONFIG.STUDENT_COLS.CLASSES_COMPLETED] = 0;
    newRow[CONFIG.STUDENT_COLS.PAYMENT_STATUS]    = 'Pending';
    newRow[CONFIG.STUDENT_COLS.AMOUNT_PAID]       = 0;
    newRow[CONFIG.STUDENT_COLS.ENROLLED_ON]       = new Date();
    newRow[CONFIG.STUDENT_COLS.STATUS]            = 'Active';
    newRow[CONFIG.STUDENT_COLS.TOTAL_CLASSES] = totalClasses;
    newRow[CONFIG.STUDENT_COLS.TOTAL_AMOUNT]  = totalAmount;

    studentSheet.appendRow(newRow);
    studentSheet.getRange(studentSheet.getLastRow(), CONFIG.STUDENT_COLS.ENROLLED_ON + 1)
        .setNumberFormat('dd-MMM-yyyy');

    Logger.log('Regular student record created: ' + reference);
}
...
function onOpen() {
    ...
    ui.createMenu('🎠 Kings Equestrian')
        ...
        .addToUi()
        .addItem('📅 Fix Missing Calendar Events', 'fixMissingCalendarEvents')
        .addItem('🔁 Process Reschedule Requests', 'processAllRescheduleRequests');
}
