...
function sendWelcomeEmail(data) {
  ...
  const serviceList = Array.isArray(data.services)
    ...
    : String(data.services || '').split(',').map(s => s.trim()).filter(Boolean);
  ...
  const servicesHTML = serviceList.map(s => 
  ...
  const serviceDetailsHTML = `
    <div style="margin: 15px 0; padding: 15px; background: #f9f9f9; border-radius: 8px;">
      ...
      <p style="font-size: 12px; color: #666; margin-top: 10px; font-style: italic;">
        See attached PDFs for detailed service information
      </p>
    </div>
  `;
...
function SendPaymentReceipt() {
  ...
  if (!paymentSheet || !bookingSheet || !paymentLogSheet) {
    ui.alert('Required sheets not found. Please check:\n- ' + CONFIG.SHEETS.BOOKING_FORM + '\n- ' + CONFIG.SHEETS.PAYMENT_FORM + '\n- ' + CONFIG.SHEETS.PAYMENT_LOG);
    return;
  }
  ...
  for (let i = 0; i < numRows; i++) {
    ...
    try {
      ...
      for (let j = 1; j < bookingValues.length; j++) {
        if (String(bookingValues[j][CONFIG.BOOKING_COLS.REFERENCE] || '').trim() === 
...
if (driveInfo) {
  ...
  if (CONFIG.PAYMENT_COLS.PAYMENT_RECEIPT_DRIVER_LINK === undefined) {
    const lastCol = paymentSheet.getLastColumn();
    paymentSheet.getRange(1, lastCol + 1).setValue('Receipt Drive Link');
  }
  
  const driveLinkCol = CONFIG.PAYMENT_COLS.PAYMENT_RECEIPT_DRIVER_LINK !== undefined 
    ? CONFIG.PAYMENT_COLS.PAYMENT_RECEIPT_DRIVER_LINK
    : paymentSheet.getLastColumn() - 1;
    
  paymentSheet.getRange(rowIndex, driveLinkCol + 1).setValue(driveInfo.fileUrl);
}
...
<body>
  <div class="container">
    ...
    <div class="footer">
      ...
      failCount++;
      const errorMsg = `Row ${rowIndex} (Ref: ${referenceNumber || 'N/A'}, Email: ${email || 'N/A'}): ${error.message}`;
...
function setupTriggers() {
  ...
  // Create new trigger for booking form submissions
  const ss = SpreadsheetApp.getActiveSpreadsheet();
